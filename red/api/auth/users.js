...
function retrieveToken(userName) {
    return when.resolve(null);
}
...
function init(config) {
    ...
    if (config.type == "credentials") {
        ...
        if (config.retrieveToken && typeof config.retrieveToken === "function") {
            api.retrieveToken = config.retrieveToken;
        } else {
            api.retrieveToken = retrieveToken;
        }
        if (config.addToken && typeof config.addToken === "function") {
            api.addToken = config.addToken;
        } else {
            api.addToken = addToken;
        }
    }
    if (config.default) {
...
module.exports = {
    ...
    retrieveToken: function(username) { return api.retrieveToken(username) },
    addToken: function(user) { return api.addToken(user) }
};
